  ...
  return (
    <header className="sticky top-0 z-50 backdrop-blur-md bg-[#0d1117]/90 border-b border-gray-800 px-6 py-4 flex items-center justify-between">
      ...
      <nav className="hidden md:flex items-center space-x-5 text-xs font-semibold text-gray-300">
        <a href="/shelters" className="hover:text-emerald-400 transition flex items-center space-x-1">
          <MapPin className="w-3.5 h-3.5" />
          <span>Shelters</span>
        </a>
        <a href="/risk-map" className="hover:text-emerald-400 transition flex items-center space-x-1">
          <Layers className="w-3.5 h-3.5" />
          <span>Risk Map</span>
        </a>
        <a href="/evacuation" className="hover:text-emerald-400 transition flex items-center space-x-1">
          <ShieldAlert className="w-3.5 h-3.5" />
          <span>Evacuation</span>
        </a>
        {token && (user?.role === "COORDINATOR" || user?.role === "ADMIN") && (
          <a href="/dashboard" className="hover:text-emerald-400 transition flex items-center space-x-1">
      ...
      <div className="flex items-center space-x-3">
        ...
        {token ? (
          ...
          <a
            href="/"
            className="px-4 py-2 text-xs font-semibold rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white transition shadow-md"
          >
            Citizen Sign In
          </a>
        )}
